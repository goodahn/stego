...
def embed_message(image_path, key, message):
    ...
    for i in range(BLOCK_NUMBER):
        s_imgs.append(img[i*BLOCK_HEIGHT:(i+1)*BLOCK_HEIGHT, 0:w])
    ...
    r_num = random.sample(range(0,BLOCK_NUMBER),BLOCK_NUMBER)
    ...
    while len(message_bits) > 0 and idx < len(r_num):
        ...
        invert_MSE /= float(w) * BLOCK_HEIGHT
        output[-1].append(s)
        ...
        invert_gray_MSE /= float(w) * BLOCK_HEIGHT 
        output[-1].append(s)
        ...
        print MSE, invert_MSE, gray_MSE, invert_gray_MSE
    ...
    bordersize = 2
    result = copy.deepcopy(s_imgs[0])
    if i in r_num[:len(KEYS)]:
        s_imgs[0] = cv2.copyMakeBorder(s_imgs[0][bordersize:BLOCK_HEIGHT-bordersize,bordersize:w-bordersize], top=bordersize, bottom=bordersize, left=bordersize, right=bordersize, borderType= cv2.BORDER_CONSTANT, value=[255,0,0] )
    result_border = s_imgs[0]
    for i in range(BLOCK_NUMBER-1):
        result = np.vstack((result,s_imgs[i+1]))
        if i+1 in r_num[:len(KEYS)]:
            s_imgs[i+1] = cv2.copyMakeBorder(s_imgs[i+1][bordersize:BLOCK_HEIGHT-bordersize,bordersize:w-bordersize], top=bordersize, bottom=bordersize, left=bordersize, right=bordersize, borderType= cv2.BORDER_CONSTANT, value=[255,0,0] )
        result_border = np.vstack((result_border,s_imgs[i+1]))

    img = np.array([result[i,0:w] for i in range(h)])
    img2 = np.array([result_border[i,0:w] for i in range(h)])

    print "original message: ", message
    print "embedded message: ", encrypted

    print cv2.imwrite("stego_"+image_path[:-4]+".bmp",img.copy())
    print cv2.imwrite("stego_"+image_path[:-4]+"_border.bmp",img2.copy())
...
def detect_message(image_path, key, secret):
    ...
    encoded_message = encoded_message.split()[0]
    print "encoded message:",encoded_message
    print "original message:",aes_cipher.decrypt(encoded_message[:-(len(encoded_message)%16)])
    return img
